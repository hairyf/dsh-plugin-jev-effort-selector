 ...
 * Registrations:
 *
 * 1. The `jev-effort-selector` settings namespace. The schema declared here
 *    validates every field and the harness persists edits into the user
 *    settings document — the plugin owns no storage of its own. The form the
 *    user sees is the browser half's card on `settings.plugin.item`; the
 *    harness does not render one from the schema.
...
/** Durable settings schema; the harness validates and persists it, the browser half's card edits it. */
