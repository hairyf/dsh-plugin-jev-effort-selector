 ...
 *    storage of its own.
 * 2. The `jevEffort` session projection: a fold over the harness's own
 *    `request/header` events, which carry the effort each request actually
 *    used. The browser half reads it through the standard `useProjection`
 *    prop, so the chip needs no polling and no RPC, and each session sees
 *    only its own requests. This plugin appends NOTHING to the session log.
...
function apply(ctx) {
  ...
  // The fold reads the harness's OWN `request/header` event, which records the
  // exact config each request went out with — including the effort this plugin
  // rewrote. A plugin-owned event type would have been more expressive, but it
  // is unreadable: see the note at the decision site in `agent/request`.
  //
  ...
  ctx.on('agent/request', async (payload, next) => {
    ...
    // no way to set that marker. A plugin-owned event type therefore loads
    // fine in the process that wrote it and permanently bricks the log on the
    // next cold read. The effort this returns is recorded anyway: the loop
    // appends its own `request/header` event carrying the config below.
