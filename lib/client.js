...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const FIELDS = [
			...
			{ field: "apiKeyEnv", kind: "text", label: "密钥环境变量", hint: "API 密钥存放在凭据服务中的这个名字下。改名后，下方「API 密钥」的状态会跟着变。" },
		...
		/** Human names for the layers the local credentials provider reports. */
		const SOURCE_LABEL = {
			env: "启动环境变量",
			file: "凭据文件",
			"project-env": "项目 .env",
		...
		class JevForm {
			constructor(scope, credentials) {
				...
				this.failed = false;
				/**
				 * The key lives outside the settings document entirely. `info` is
				 * the describe view — configured/source/writable and nothing else —
				 * so this object never holds the secret, not even transiently after
				 * a successful write.
				 */
				this.secret = { ref: "", info: undefined, draft: "", busy: false, error: "", unreadable: false };
			...
			/** Re-describe when the saved reference name changed under us. */
			syncSecret() {
				const ref = this.savedRef();
				if (ref === this.secret.ref) return;
				this.secret = { ...this.secret, ref, info: undefined, error: "", unreadable: false };
				this.publish();
			...
			async describeSecret(ref) {
				...
				let info;
				try {
					info = (await this.credentials.describe([ref]))?.[ref];
				} catch {
					// The controller refuses a name outside the reference grammar.
					// That refusal is the authority on validity — this card does not
					// restate the grammar, it reports what the Host answered.
					info = undefined;
				}
				if (this.secret.ref !== ref) return;
				this.secret = { ...this.secret, info, unreadable: info === undefined };
				this.publish();
			}
			...
			async storeSecret() {
				...
				if (!LEGAL_API_KEY.test(value)) {
					this.secret = { ...this.secret, error: "密钥含有非法字符。" };
					this.publish();
					return;
				}
				...
				try {
					...
					this.secret = { ...this.secret, busy: false, draft: "" };
				} catch {
					// A read-only layer shadowing the reference is the common cause.
					this.secret = { ...this.secret, busy: false, error: "保存失败：该引用可能被更高优先级的层占用。" };
				}
				this.publish();
			...
			async removeSecret() {
				...
				try {
					...
					this.secret = { ...this.secret, busy: false, error: "清除失败，请重试。" };
				}
				this.publish();
				await this.describeSecret(ref);
			}
			...
			projection() {
				...
				return {
					...
					secret: {
						ref: this.secret.ref,
						known: this.secret.info !== undefined,
						/* The Host could not describe this reference — almost always
						   a name outside the reference grammar. Reported plainly
						   instead of leaving the row stuck on "reading status". */
						unreadable: this.secret.unreadable,
						configured: this.secret.info?.configured === true,
						source: this.secret.info?.source,
						/* A reference supplied by a read-only layer cannot be written
						   from here; the stock page shows that rather than failing a save. */
						writable: this.secret.info?.writable === true,
		 ...
		 *
		 * The value is never read back: the control renders only whether the
		 * reference resolves, which layer supplies it, and whether this client
		 * may write it. A reference the launching environment supplies is
		 * reported as read-only instead of accepting a write that resolution
		 * would then ignore.
		...
		function SecretField(props) {
			...
			let status;
			if (secret.ref === "") status = "请先填写上方的「密钥环境变量」。";
			else if (secret.unreadable) status = "无法读取该引用的状态：请检查「密钥环境变量」是否为合法的环境变量名（字母或下划线开头）。";
			else if (!secret.known) status = "正在读取状态…";
			else if (!secret.configured) status = "未配置。";
			else {
				const source = SOURCE_LABEL[secret.source] ?? secret.source ?? "未知来源";
				status = secret.writable
					? `已配置（来源：${source}）。`
					: `已配置（来源：${source}）。该层只读，请在其来源处修改。`;
			}

			const shadowed = secret.known && secret.configured && !secret.writable;
			const locked = disabled || secret.busy || secret.ref === "" || secret.unreadable || shadowed;
			...
			return React.createElement("div", { style: cardCSS.field },
				...
				React.createElement("div", { style: cardCSS.secretRow },
					React.createElement("input", {
						...
						value: secret.draft,
						disabled: locked,
						placeholder: secret.configured ? "输入新密钥以替换" : "输入 API 密钥",
					...
					secret.configured && secret.writable
						? React.createElement("button", {
				...
				secret.error !== ""
					...
					: React.createElement("p", { style: cardCSS.hint }, status),
				React.createElement("p", { style: cardCSS.hint },
					"密钥由凭据服务保管（~/.dsh/.credentials.yaml，权限 0600），不写入设置文件，也不会回传到浏览器。"));
		}
				...
				open
					? React.createElement("div", { style: cardCSS.body },
						...
						FIELDS.map((spec) => {
							...
							// The key row follows the reference that names it, so the
							// two read as one setting rather than two unrelated boxes.
							if (spec.field !== "apiKeyEnv") return row;
