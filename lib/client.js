 ...
 * SRC-mode Remote, and re-fetches whenever the `jevEffort` projection moves —
 * that projection folds the harness's own `request/header`, so it changes
 * exactly when a request went out. No polling, and the chip never shows
 * another session's value because everything is keyed by `sessionId`.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const REASONS = {
			"new-topic": "新话题，由 Jev 判断",
			jev: "由 Jev 判断",
			continues: "延续上一任务，由 Jev 判断",
		...
		function EffortChip(props) {
			...
			const trigger = props.useProjection(PROJECTION_KEY);
			const [state, setState] = React.useState(null);
			const [open, setOpen] = React.useState(false);
			const [busy, setBusy] = React.useState(false);
			const sessionId = typeof props.sessionId === "string" ? props.sessionId : "";
			const connection = props.useJevConnection((c) => c);
			const triggerAt = trigger && typeof trigger.at === "number" ? trigger.at : 0;
			...
			const reasonText = sessionOn && decision ? (REASONS[decision.reason] || "") : "";
