 ...
 *
 * It also registers the plugin's settings card on `settings.plugin.item`. The
 * host half declares the schema and the harness persists the section, but the
 * Plugins tab shows only namespaces that also have a card — registering the
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			/* Not decided by Jev this turn (a failure), or Jev off for this
			   session: the whole chip takes the colour the stock trigger uses
			   when it is disabled. */
			".dshjev-chip.dshjev-chipMuted,.dshjev-chipMuted .dshjev-effort.dshjev-effort{color:var(--dsw-alias-label-dimmed)}",
		].join("");
		...
		/**
		 * Reasons meaning Jev did NOT decide this turn. The chip goes muted for
		 * these: the effort shown is the one in force, not Jev's choice.
		 */
		...
		const FIELDS = [
			...
			{ field: "apiUrl", kind: "text", label: "API 地址", hint: "Jev System One 接口的完整地址。未填写时不会调用 Jev。", placeholder: "输入 Jev System One 接口地址" },
		...
		function EffortChip(props) {
			...
			if (!sessionOn) {
				...
				muted = NOT_DECIDED.indexOf(decision.reason) !== -1;
				body = [
					React.createElement("span", { key: "n" }, "Jev"),
					React.createElement("span", { key: "l", className: effortClass(effort) }, effort === "" ? "—" : label(effort)),
				];
				if (pct !== null && !muted) body.push(React.createElement("span", { key: "c", className: "dshjev-effort" }, pct));
			}
			...
			return React.createElement("div", { ref: rootRef, className: "dshjev-chipRoot" },
				React.createElement("button", {
					type: "button",
					className: muted ? "dshjev-chip dshjev-chipMuted" : "dshjev-chip",
					title,
					"aria-expanded": open,
					onClick: () => setOpen((v) => !v),
				}, ...body),
			...
			async save() {
				...
				if (this.levelEdits.size > 0) {
					try {
						...
						if (Object.keys(next).length === 0) await this.scope.unset("levels");
						else await this.scope.set("levels", next);
						this.levelEdits.clear();
					} catch {
						landed = false;
					}
		...
		function LevelsSection(props) {
								...
								return React.createElement("div", { key: model.route, style: cardCSS.modelRow, "data-route": model.route },
									...
									model.tooFew
										...
										: model.tooMany
											...
											: React.createElement("p", { style: cardCSS.hint },
												custom ? "Jev 只会在选中的档位里挑选。" : "自动：高亮的是插件按这个模型支持的档位推导出的几档。"));
							}))),
			...
			return React.createElement("div", { style: cardCSS.field },
				...
				open ? null : React.createElement("p", { style: cardCSS.hint }, "为每个模型指定 Jev 可以选择的推理档位，默认自动。"),
				body);
		}
			...
			return React.createElement("li", { className: open ? "dshjev-card dshjev-cardOpen" : "dshjev-card" },
				...
				open
					? React.createElement("div", { style: cardCSS.body },
						...
						React.createElement(LevelsSection, { levels: state.levels, actions: props, disabled }),
