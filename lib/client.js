 ...
 * selector: the reasoning effort the latest request in this session used.
 *
 * The value arrives through the standard `useProjection` prop, reading the
 * `jevEffort` projection the host half folds from the harness's own
 * `request/header` events. That makes the chip session-scoped for free — no
 * polling, no RPC, and no chance of showing another conversation's value.
 *
 * The chip is hidden while the plugin is switched off. Its source is a harness
 * event that fires for every request, so it would otherwise keep reporting the
 * effort you picked by hand — restating the model selector one control to its
 * right, and implying Jev is still choosing when it is not.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * The composer chip: "思考 · Medium".
		 *
		 * The projection folds the harness's `request/header` events, so this
		 * is the effort the latest request actually carried. It deliberately
		 * does NOT claim Jev chose it, and shows no confidence: the fold reads
		 * the request, not the decision behind it.
		 *
		 * Because that source is indifferent to this plugin, the switch has to
		 * be consulted separately — with Jev off the same event still carries
		 * the effort you selected by hand, which is not this chip's subject.
		 *
		 * Renders nothing until this session has issued a request that named an
		 * effort, so a fresh conversation shows no placeholder.
		 * @param props - session-scoped standard props from the slot owner.
		 */
		function EffortChip(props) {
			/* Both hooks run on every render: React fixes hook order, so the
			   off switch must not short-circuit ahead of `useProjection`. */
			const enabled = props.useJevEnabled((on) => on);
			const latest = props.useProjection(PROJECTION_KEY);
			if (!enabled) return null;
			if (latest === undefined || latest === null) return null;

			const choice = typeof latest.choice === "string" ? latest.choice : "";
			if (choice === "") return null;

			return React.createElement("span", { style: chipStyle, title: "上一次请求使用的推理等级" },
				React.createElement("span", { style: nameStyle }, "思考"),
			...
			ctx.slots.inject("conversation.input.right", () => ctx.slots.register(
				{
					...
					id: "jev-effort",
					order: 50,
					inject: () => ({ hooks: { jevEnabled: enabledStore } }),
				},
				EffortChip,
			));
