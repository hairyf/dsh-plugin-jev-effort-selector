	...
	factory: (require) => {
		...
		const CSS = [
			...
			/* Not decided by Jev this turn (a failure), or Jev off for this
			   ...
			   when it is disabled. */
			".dshjev-chipMuted,.dshjev-chipMuted .dshjev-effort{color:var(--dsw-alias-label-dimmed)}",
		].join("");
		...
		function EffortChip(props) {
			...
			if (!sessionOn) {
				...
				body = [
					React.createElement("span", { key: "n" }, "Jev"),
					React.createElement("span", { key: "l", className: "dshjev-effort" }, effort === "" ? "—" : label(effort)),
				];
